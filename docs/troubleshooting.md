...
| --- | --- |
| Ghidra cannot be found | Set `GHIDRA_INSTALL_DIR` to the extracted distribution root, or pass `--ghidra-path`; configure it in the stdio client's environment |
...
| HTTP client cannot connect | Confirm transport, host, port, `/mcp` path, and server logs; `/sse` requires `--transport sse` |
