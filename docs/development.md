...
- The Docker image and `scripts/ghidra_release.env` pin the decompiler natives overlay to a specific release tag (currently `v0.1.5-rc.1`). Bump the URL and SHA256 in both places only when a release changes the natives themselves.
- Before tagging, run `release-decompiler-natives` with `workflow_dispatch`, verify its `mecha-ghidra-release-assets` artifact, and record that run in `MECHA_GHIDRA_RELEASE_NATIVE_ASSET_RUN_ID`. Tag builds reuse those verified native ZIPs so the published archive SHA256 is exactly the value pinned by Docker; the Python source distribution is still rebuilt from the tag.
