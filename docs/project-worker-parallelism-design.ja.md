...
2026-09-21作成、2026-09-22改訂。基準コミット: `ef73f5f`（`release/v1.0.0`）。現行コード・公式資料の確認と設計相談を反映した**設計書**。実装、サービスの起動・設定変更、並列動作の実機検証は行っていない。
...
- 受け取った指摘をローカルの現行コード、Ghidra公式文書、Ghidra 12.1.3同梱Serverソースと照合した。
