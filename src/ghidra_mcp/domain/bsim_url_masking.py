...
_BSIM_URL_RE = re.compile(_BSIM_URL_START_RE + r"(?:(?![,;)(]" + _BSIM_URL_START_RE + r")[^\s<>])+")
...
def _mask_raw_url_query(text: str) -> str:
    ...
    return (
        url_without_fragment[: query_start + 1] + _mask_query_values(url_without_fragment[query_start + 1 :]) + fragment
    )
...
def mask_bsim_urls_in_text(message: str) -> str:
    ...
    def mask_match(match: re.Match[str]) -> str:
        ...
        if preceding in {"'", '"'}:
            ...
            if closing >= 0 and all(character in _TRAILING_QUOTE_PUNCTUATION for character in token[closing + 1 :]):
